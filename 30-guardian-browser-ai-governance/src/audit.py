"""GUARDIAN — SHA-256 hash-chained append-only audit log.

Pattern lifted from apps/10-sentinel. Each entry includes:
  - prev_hash      : entry_hash of the previous record (or 64x"0" genesis)
  - timestamp_utc  : ISO8601
  - entry_hash     : sha256(json(body+prev_hash+timestamp_utc))

Forensics-grade: any tamper on a middle row breaks the chain and is detectable
in O(N) verification.
"""
...
AUDIT_LOG = AUDIT_DIR / "guardian_audit.jsonl"
...
def append_audit(entry: dict) -> dict:
    """Append a chained audit entry. Returns the written record."""
...
def verify_chain() -> tuple[bool, int, str]:
    """Return (ok, rows_checked, message). Walks the file end-to-end."""
    if not AUDIT_LOG.exists():
...
def reset_audit_log() -> None:
    """Wipe the audit log (used on app cold-start to keep demos clean)."""
    if AUDIT_LOG.exists():
        AUDIT_LOG.unlink()
